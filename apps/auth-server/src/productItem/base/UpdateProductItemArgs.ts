/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProductItemWhereUniqueInput } from "./ProductItemWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ProductItemUpdateInput } from "./ProductItemUpdateInput";

@ArgsType()
class UpdateProductItemArgs {
  @ApiProperty({
    required: true,
    type: () => ProductItemWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ProductItemWhereUniqueInput)
  @Field(() => ProductItemWhereUniqueInput, { nullable: false })
  where!: ProductItemWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ProductItemUpdateInput,
  })
  @ValidateNested()
  @Type(() => ProductItemUpdateInput)
  @Field(() => ProductItemUpdateInput, { nullable: false })
  data!: ProductItemUpdateInput;
}

export { UpdateProductItemArgs as UpdateProductItemArgs };
